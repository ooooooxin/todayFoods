...
import { injectPageScrollBridgePlugin } from './vite-plugins/inject-page-scroll-bridge'
import syncManifestPlugin from './vite-plugins/sync-manifest-plugins'
import { vueSSRPatchPlugin } from './vite-plugins/vue-ssr-patch'
...
export default defineConfig(({ command, mode }) => {
  ...
  return defineConfig({
    ...
    plugins: [
      // 为每个路由页注入滚动同步（onPageScroll 仅在页面组件内生效，App.ku 无法监听）
      injectPageScrollBridgePlugin(),
      // UniXXX 需要在 Uni 之前引入
      UniLayouts(),
      UniPlatform(),
      UniManifest(),
      ...
      Uni(),
      // Vue SSR 补丁插件 - 解决 uni-app 405+ 与 Vue 3.4 的兼容问题
      vueSSRPatchPlugin(),
      {
      ...
      openDevTools(),
      // 构建完成提示插件 - 输出正确的构建路径
      command === 'build' && UNI_PLATFORM?.startsWith('mp-') && {
        name: 'build-output-hint',
        closeBundle() {
          const outputDir = process.env.UNI_OUTPUT_DIR || `dist/build/${UNI_PLATFORM}`
          const platformName = UNI_PLATFORM === 'mp-toutiao' ? '抖音' : UNI_PLATFORM === 'mp-weixin' ? '微信' : UNI_PLATFORM === 'mp-alipay' ? '支付宝' : ''
          console.log(`\n✅ 构建完成！请打开${platformName}开发者工具，导入项目: ${path.resolve(process.cwd(), outputDir)}`)
        },
      },
    ],
    define: {
    ...
    build: {
      ...
      // 根据 mode 区分输出目录，test 环境打包到 dist/build-test，production 打包到 dist/build-prod
      outDir: mode === 'test' ? 'dist/build-test' : mode === 'production' ? 'dist/build-prod' : 'dist/build',
    },
  })
})
