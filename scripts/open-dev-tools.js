...
function _openDevTools() {
  ...
  const uniPlatformText = UNI_PLATFORM === 'mp-weixin' ? '微信小程序' : UNI_PLATFORM === 'mp-alipay' ? '支付宝小程序' : UNI_PLATFORM === 'mp-toutiao' ? '抖音小程序' : '小程序'
  ...
  if (platform === 'darwin') {
    ...
    if (UNI_PLATFORM === 'mp-weixin') {
      ...
      // 抖音小程序开发者工具路径（需要根据实际情况调整）
      command = `"C:\\Users\\%USERNAME\\AppData\\Local\\抖音开发者工具\\抖音开发者工具.exe" --project="${projectPath}"`
    }
  }
  else {
    // Linux 或其他系统
...
export default function openDevTools() {
  ...
  // 支持自动打开开发者工具的平台
  const supportedPlatforms = ['mp-weixin', 'mp-alipay', 'mp-toutiao']

  return {
    name: 'uni-devtools',
    writeBundle() {
      if (isFirstBuild && supportedPlatforms.includes(process.env.UNI_PLATFORM || '')) {
        isFirstBuild = false
        _openDevTools()
      }
    },
