由 `uniapp` + `Vue3` + `Ts` + `Vite5` + `UnoCss` + `wot-ui` + `z-paging` 构成，使用了最新的前端技术栈，无需依靠 `HBuilderX`，通过命令行方式运行 `web`、`小程序` 和 `App`（编辑器推荐 `VSCode`，可选 `webstorm`）。
...
执行 `pnpm dev:mp-toutiao` 运行 `抖音小程序`

## 📦 运行（支持热更新）

- web平台： `pnpm dev:h5`, 然后打开 [http://localhost:9000/](http://localhost:9000/)。
- weixin平台：`pnpm dev:mp` 然后打开微信开发者工具，导入本地文件夹，选择本项目的`dist/dev/mp-weixin` 文件。
- toutiao平台：`pnpm dev:mp-toutiao` 然后打开抖音开发者工具，导入本地文件夹，选择本项目的`dist/dev/mp-toutiao` 文件。
...
- toutiao平台：`pnpm build:mp-toutiao`, 打包后的文件在 `dist/build/mp-toutiao`，然后通过抖音开发者工具导入，并点击右上角的“上传”按钮进行上传。
